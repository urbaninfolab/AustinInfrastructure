/**
 * @file Created under Dr. Jiao's Urban Information Lab,
 * this file builds the 2D map for transportation-related facilities in Austin.
 */
	...
	let sampleData = [
		{
			...
			description:
				'6780-8398 N US 183 | NON - ADVISED INCIDENT | 16:48:52',
	...
	for (city in cities) {
		for (let i = 0; i < dateArray.length; i++) {
			try {
				...
				let jsonUrl =
					'../data/' + date + '-FireMap' + cities[city] + '.json'
...
function processData(data) {
	...
	let angles = [
		0, 25, 45, 65, 90, 115, 135, 155, 180, 205, 225, 245, 270, 295, 315,
		335,
	]
...
function addMapLayer(map) {
	...
	if (inactive_flag) map.addLayer(inactiveFires)
	if (purple_air_diaplay_flag && microsoft_air_display_flag)
		map.addLayer(markers)
	else if (purple_air_diaplay_flag) map.addLayer(purpleAirMonitors)
	else map.addLayer(microsoftAirMonitors)
}
	...
	for (var i in onehourForecastGroup._layers) {
		for (var j in onehourForecastGroup._layers[i]._layers) {
			for (var k in onehourForecastGroup._layers[i]._layers[j]._layers) {
				...
				var inSmoke = onehourForecastGroup._layers[i]._layers[
					j
				]._layers[k]
					.getBounds()
					.contains(current_position._latlng)
				//console.log(inSmoke)
				if (inSmoke) {
					//console.log(it);
					warningLevel =
						it == 0
							? 'a Moderate'
							: it == 1
							? 'an Unhealthy for Sensitive Groups'
							: it == 2
							? 'an Unhealthy'
							: it == 3
							? 'a Very Unhealthy'
...
function buildSelectBar(map) {
	const checklist = document.getElementById('filter-menu')
			...
			shpfile = new L.Shapefile(shapefileName, {
				onEachFeature: function (feature, layer) {
					popupContent = `
						...
						`
					layer.bindPopup(popupContent)
					let hvi_exposure = parseFloat(
						feature.properties['HVI_Exposu']
					)
				...
				shpfile = new L.Shapefile(shapefileName, {
					onEachFeature: function (feature, layer) {
						var averageNum =
							result[
								cities[
									feature.properties['ctid'].substring(0, 5)
								]
							]
...
async function mapPurpleAirData(map) {
	...
	if (airData.length == 0) {
		...
		for (city in cities) {
			...
			let jsonUrl =
				'https://api.purpleair.com/v1/sensors?api_key=81D9ACDC-966F-11EC-B9BF-42010A800003&nwlat=' +
				latlng[0] +
				'&selat=' +
				latlng[1] +
				'&nwlng=' +
				...
				'&selng=' +
				latlng[3] +
				'&fields=latitude,longitude,altitude,pm2.5_10minute'
			let response = await fetch(jsonUrl)
			let currentData = await response.json()
	...
	for (let i = 0; i < airData.length; i++) {
		...
		let airApiUrl =
			...
			sensorKey +
			'?api_key=81D9ACDC-966F-11EC-B9BF-42010A800003'
...
async function mapTransitData(map) {
	...
	url =
		'https://smartcity.tacc.utexas.edu/data/transportation/transitposition.json'
...
async function mapMicrosoftAirData(map) {
	...
	for (let i = 0; i < selectedData.length; i++) {
		...
		var circleMarker = new L.marker(
			[popupData.Latitude, popupData.Longitude],
			{
				icon: L.divIcon({
					html:
						'<div><span><b>' +
						(Math.round(pm10Mins * 100) / 100).toFixed(2) +
						'</b></span></div>',
					className: 'marker-cluster' + c,
					iconSize: new L.Point(40, 40),
				}),
...
function buildMicrosoftAirDataPopup(marker, popupData, description) {
	...
	var airMarkerPopup = `
			...
			<span>Temperture: ${(Math.round(popupData.TempC * 100) / 100).toFixed(
				2
			)} </span><BR>
			<span>Humidity: ${(Math.round(popupData.Humidity * 100) / 100).toFixed(
				2
			)} </span><BR>
		...
		.then((data) => {
			...
			for (var i = 1; i < lines.length; i++) {
				...
				for (var j = 0; j < headers.length; j++) {
					if (currentline[j] == undefined) valid = false
					obj[headers[j]] = currentline[j]
				}
				if (valid) jsonResult.push(obj)
			}
			...
			for (var i = 0; i < jsonResult.length; i++) {
				...
				if (!fireDept && !policeDept && !hospital) continue
				else if (
					!fireDept &&
					!policeDept &&
					hospital &&
				...
				var marker = L.marker([jsonResult[i].Y, jsonResult[i].X]).addTo(
					map
				)
	...
	if (transit) {
		...
		fetch(
			...
			.then((transit_json) => {
				...
				for (let i = 0; i < transit_json['entity'].length; i++) {
					if (
						!transit_json['entity'][i]['vehicle'].hasOwnProperty(
							'trip'
						)
					) {
						continue
					...
					let y =
						transit_json['entity'][i]['vehicle']['position'][
							'latitude'
						]
					let x =
						transit_json['entity'][i]['vehicle']['position'][
					...
					var route_id =
						transit_json['entity'][i]['vehicle']['trip']['routeId']
					var vehicle_id = transit_json['entity'][i]['id']
					var speed =
						transit_json['entity'][i]['vehicle']['position'][
							'speed'
...
function buildScooterMap() {
	...
	if (micromobility) {
		...
		fetch(
			'https://smartcity.tacc.utexas.edu/data/transportation/freebike.json'
		)
			.then((response) => {
			...
			.then((scooter_json) => {
				...
				for (let i = 0; i < scooter_json['data']['bikes'].length; i++) {
					...
					var bike_id = scooter_json['data']['bikes'][i]['bike_id']
					var bike_type =
						scooter_json['data']['bikes'][i]['vehicle_type_id']
					scooter_marker.bindPopup(
						' ID: ' + bike_id + ', Type: ' + bike_type
					)
...
function buildLiveIncidentMap() {
	...
	if (incident) {
		...
		fetch(
			'https://smartcity.tacc.utexas.edu/data/transportation/incident_active.json'
		)
			.then((response) => {
				// Check if the response is ok (status code in the range 200-299)
				if (!response.ok) {
...
function buildArchivedIncidentMap() {
	...
	if (archived_incident) {
		...
		fetch(
			'https://smartcity.tacc.utexas.edu/data/transportation/incident_archived.json'
		)
			.then((response) => {
				// Check if the response is ok (status code in the range 200-299)
				if (!response.ok) {
			...
			.then((archived_incident_json) => {
				...
				for (let i = 0; i < archived_incident_json.length; i++) {
					...
					archived_incident_marker.setIcon(
						L.icon({
							iconUrl: iconLink,
							iconSize: [22, 32],
							iconAnchor: [11, 32],
					...
					archived_incident_marker.bindPopup(
						' Issue: ' +
							issue +
							', Address: ' +
							address +
...
/**
 * Creates a layer of incident choropleth using shp file
 * @author Yiming Xu
 */
...
/* let altFuelLayer = null

function buildAltFuelMap(checked) {
	if (!checked && altFuelLayer) {
		map.removeLayer(altFuelLayer)
		altFuelLayer = null
	} else {
		let shapefile_path = "./data/altFuel.zip"
		altFuelLayer = new L.Shapefile(shapefile_path, {
			onEachFeature: function (feature, layer) {
				let popupContent = `
				<div class="basic-info">
					<span>Station Name: ${feature.properties["Station Na"]}</span><BR>
					<span>Street Address: ${feature.properties["Street Add"]}</span><BR>
					<span>ZIP Code: ${feature.properties["ZIP"]}</span><BR>
				</div>
			`
				...
				layer.options.fillOpacity = .65
			}
		})
		// console.log(altFuelLayer)
		altFuelLayer.addTo(map);
	}
} */
 ...
 * @author Kay Kong <lykong@utexas.edu>
 */
async function onElecClick(checked) {
...
const GOOGLE_API_KEY = 'AIzaSyAsCvHmPnfp7EleZJU3Fmd1-Jwi7-7Mc-w'
...
/**
 * Creates a layer of cell tower markers from .geojson file
 * @param {Boolean} checked Status of checkbox
 * @author Kay Kong <lykong@utexas.edu>
 */
async function onCellClick(checked) {
	if (!checked && cellLayer) {
		map.removeLayer(cellLayer)
		cellLayer = null
	} else {
		let data = await fetch(`${DATA_DIR}cell.geojson`)
		data = await data.json()
		cellLayer = new L.geoJson(data).addTo(map)
	}
}
...
function buildDropdownMenu(map) {
	...
	checkbox.addEventListener('click', () => {
		console.log('electric_charging click')
		// buildAltFuelMap(checkbox.clicked);
		onElecClick(checkbox.clicked)
	})

	checkbox = document.querySelector('.other_fuel')
	checkbox.addEventListener('click', () => {
		console.log('other_fuel click')
		onOtherFuelClick(checkbox.clicked)
	})

	checkbox = document.querySelector('.cell')
	checkbox.addEventListener('click', () => {
		console.log('cell click')
		onCellClick(checkbox.clicked)
	})
}
...
function addShapefileRadioListener(map) {
	...
	for (var i = 0, max = radios.length; i < max; i++) {
		radios[i].onclick = function () {
			...
			mapFireIncident(map, dateArray, inactive_flag, shapefile_display_flag, purple_air_diaplay_flag, microsoft_air_display_flag);
			*/
			if (currentShapefile != null) map.removeLayer(currentShapefile)
			buildShapefile(map, shapefile_display_flag)
...
document.getElementsByClassName('leaflet-legend-list')[0].style =
	'text-align: left;'
